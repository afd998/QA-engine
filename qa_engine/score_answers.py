...
def score_all_answers(gold, pred):
    ...
    for row in gold.itertuples():
        ...
        golds = [ans.strip() for ans in row.answer.lower().split("|")]
        scores = {"p": [], "r": [], "f": []}
        for i, gold_answer in enumerate(golds):
            gold_words = set(nltk.word_tokenize(gold_answer))
            pred_answer = pred.loc[row.Index]
            pred_words = set(nltk.word_tokenize(pred_answer.answer.lower().strip()))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-answer", dest='answer_fname', help='Answer key file')
    parser.add_argument("-response", dest='response_fname', help='Your response file')
